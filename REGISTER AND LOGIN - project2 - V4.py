...
INVALID_EMAIL = 'Invalid email format. \nPlease enter a valid email address no more than 64 characters.'
...
def get_login_information():
    ...
    """      
    login_id = input("Enter your username or email:").strip()
...
def login_options():
    ...
    """
    while True:
        login_or_register = input(LOGIN_OR_REGISTER).lower().strip()
        ...
        if login_or_register == "login":
            ...
            for user in users:
                if (user.username.lower() == login_id.lower() or user.email.lower() == login_id.lower()) and user.password == password:
